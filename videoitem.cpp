...
//rtsp://admin:admin1234@10.3.1.34:554/ISAPI/streaming/channels/502
void VideoItem::componentComplete()
{
    ...
    g_object_set(m_videoPipe->videoSink, "widget", videoItem, nullptr);
    QTimer::singleShot(2000, [=](){GST_DEBUG_BIN_TO_DOT_FILE(GST_BIN(m_videoPipe->pipeline), GST_DEBUG_GRAPH_SHOW_ALL, "pipe0");});
    // needed for proper OpenGL context setup for GStreamer elements (QtQuick renderer)
    auto setRenderer = [videoItem, this](QQuickWindow *window) {
        if (window) {
...
//                qWarning()
//                    << "GStreamer error: while setting renderer: pending state change failure";
//                return;
//            }
...
//            Q_ASSERT(sink);
            ...
            m_videoPipe->videoSink = GST_ELEMENT(gst_object_ref(m_videoPipe->videoSink));
            connect(window, &QQuickWindow::sceneGraphInvalidated,
                    [=] { gst_object_unref(m_videoPipe->videoSink); });
//            g_object_set(m_videoPipe->videoSink, "sink", sink, nullptr);
            window->scheduleRenderJob(new RenderJob([=] {
//                                          g_object_set(m_videoPipe->videoSink, "sink", sink, nullptr);
//                                          m_videoPipe->renderPad = gst_element_get_static_pad(sink, "sink");
    ...
    if (videoItem->window())
        setRenderer(videoItem->window());
    connect(videoItem, &QQuickItem::windowChanged, videoItem, setRenderer);
}
...
void VideoItem::createPipeline()
{
    ...
    m_videoPipe->videoSink = gst_element_factory_make("qml6glsink", nullptr);
    m_videoPipe->flip = gst_element_factory_make("videoconvert", nullptr);
...
//    GstElement *fakesink = gst_element_factory_make("fakesink", nullptr);
    ...
    m_videoPipe->gload = gst_element_factory_make("glupload", nullptr);
//    g_object_set(m_videoPipe->videoSink, "sink", fakesink, nullptr);
    ...
    if(!gst_bin_add(GST_BIN(m_videoPipe->pipeline), m_videoPipe->src)) {
        qDebug() << "src src src pipeline elements failed";
    }
    if(!gst_bin_add(GST_BIN(m_videoPipe->pipeline), m_videoPipe->videoDecode)) {
        qDebug() << "videoDecode videoDecode videoDecode pipeline elements failed";
    }
    gst_bin_add_many(GST_BIN(m_videoPipe->pipeline),m_videoPipe->flip,m_videoPipe->gload, m_videoPipe->videoSink, nullptr);

    if (!gst_element_link_many(m_videoPipe->flip,m_videoPipe->gload, /*gload,*/ m_videoPipe->videoSink, nullptr))
